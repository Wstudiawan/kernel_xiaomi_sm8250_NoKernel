...
TC="${KDIR}/.tools"
AK=${TC}/AnyKernel
KERNEL_NAME="N0kernel-KSU"
KERNEL_TYPE="EAS"
PHONE="Poco F4"
DEVICE="munch"
CONFIG=${CONFIG:-vendor/munch_defconfig}
#CODENAME="-Testing"
...
sendInfo() {
    curl -s -X POST https://api.telegram.org/bot1446507242:AAFivf422Yvh3CL7y98TJmxV1KgyKByuPzM/sendMessage -d chat_id=-1001421078455 -d "parse_mode=HTML" -d text="$(
            for POST in "${@}"; do
                echo "${POST}"
            done
        )"
